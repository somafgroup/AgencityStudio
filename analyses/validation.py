...
def validate_mapping(descriptor, *, coordinate_position: int, observable_position: int) -> tuple[dict, dict]:
    ...
    for label, column in (("coordinate", coordinate), ("observable", observable)):
        if column.get("inferred_type") != "NUMERIC":
            raise PreflightError(f"The selected {label} column must be numeric for canonical scalar analysis.")
        if int(column.get("missing_count", 0)) or int(column.get("non_numeric_count", 0)) or int(column.get("non_finite_count", 0)):
            raise PreflightError(
                f"The selected {label} column contains missing, non-numeric or non-finite values. Prepare the data explicitly before analysis."
            )
    return coordinate, observable


def _require_exact_execution_unit(source_unit: str, context_unit: str, label: str) -> list[dict]:
    source = str(source_unit or "").strip()
    context = str(context_unit or "").strip()
    if not source or not context:
        raise PreflightError(f"{label} unit metadata must be explicit before canonical execution.")
    if source == context:
        info = inspect_unit(source)
        return [] if info.recognized else [{"code": "UNKNOWN_UNIT", "message": f"Unit {source!r} is preserved but Studio cannot dimensionally verify it."}]
    compatible = units_are_compatible(source, context)
    if compatible is True:
        raise PreflightError(
            f"{label} uses {source!r} while the System context uses {context!r}. No unit conversion occurs during analysis; create explicit Prepared Data with the required unit."
        )
    if compatible is False:
        raise PreflightError(f"{label} unit {source!r} is dimensionally incompatible with System unit {context!r}.")
    raise PreflightError(
        f"{label} unit labels {source!r} and {context!r} do not match and cannot be safely verified. Prepare or document matching units before analysis."
    )


def validate_units(*, coordinate: dict, observable: dict, revision, system_observable) -> list[dict]:
    warnings: list[dict] = []
    warnings.extend(_require_exact_execution_unit(observable.get("unit", ""), system_observable.unit, "Observable"))
    warnings.extend(_require_exact_execution_unit(observable.get("unit", ""), revision.a_ref_unit, "Observable/A_ref"))
    warnings.extend(_require_exact_execution_unit(coordinate.get("unit", ""), revision.tau_unit, "Coordinate/tau"))
    if revision.w_mode == MemoryWindowMode.EXPLICIT:
        warnings.extend(_require_exact_execution_unit(coordinate.get("unit", ""), revision.w_unit, "Coordinate/w"))
    return warnings


def validate_parameter_contract(revision) -> dict:
    if revision.a_ref_value is None:
        raise PreflightError("A_ref is required for canonical execution; revise the System context explicitly.")
    if revision.tau_value is None:
        raise PreflightError("tau is required for canonical execution; revise the System context explicitly.")
    if revision.p_c_value is None:
        raise PreflightError("P_c is required for canonical execution; revise the System context explicitly.")
...
def validate_sample_contract(xi: np.ndarray, u: np.ndarray, *, requested_w: float | None, tau: float) -> None:
    """Explain public Lab/CRM coordinate constraints without altering the vectors."""
    if xi.ndim != 1 or u.ndim != 1 or len(xi) != len(u):
        raise PreflightError("Coordinate and observable must be one-dimensional arrays of equal length.")
    if len(xi) < 3:
    ...
    if not np.isfinite(xi).all() or not np.isfinite(u).all():
        raise PreflightError("Canonical scalar analysis requires finite coordinate and observable values.")
    diffs = np.diff(xi)
    if np.any(diffs <= 0):
        raise PreflightError("The selected coordinate is not strictly increasing. Prepare the data explicitly before analysis.")
    step = float(diffs[0])
    atol = float(np.finfo(float).eps * max(1.0, abs(step)) * 64.0)
    if not np.allclose(diffs, step, rtol=1e-10, atol=atol):
        raise PreflightError("The selected coordinate is irregularly sampled. Create explicit Prepared Data with resampling before canonical analysis.")
    ...
    if not math.isclose(represented, window, rel_tol=1e-9, abs_tol=float(tolerance)):
        raise PreflightError("The requested CRM window is not an integer multiple of the sampling interval. Studio will not modify w; prepare compatible data or revise the explicit scientific context.")
    if len(xi) < 2 * samples:
        raise PreflightError("The selected signal is too short for two CRM windows under this configuration.")
